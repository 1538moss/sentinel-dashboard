...
| Kuldemengde-overlegg | `❄ Kulde`-knapp (rendres når `kuldemengde_enabled`, men vises av JS kun når sesongens serie har data) slår av/på stedsbaserte etiketter med akkumulert kuldemengde per slidedato — to linjer: «❄ Lødengfjorden» + «23/47,3 (pr. 12. jan)» der målt tall er grønt når terskelen `km_needed` er passert, ellers rødt — plassert på stedets koordinat, skjules ved zoom, nullstilles ved sideinnlasting |
